# Aim: Parameter recovery Hierarchical fit Stan for first model
# here, the agent has a modified ability to learn when a choice was reported using Go compared to a choice reported using Nogo
# 6 Parameters : alpha1, beta1, lambda, perseveration, go bias, gamma1 
# gamm: modifies the learning rate accurding to the manner of reporting a selcetion (Go VS Nogo)
...
corr_param = 0
cov_param  =sqrt(alpha1_aux_var)*sqrt(beta1_aux_var)*sqrt(lambda_aux_var)*sqrt(pers_aux_var)*sqrt(gob_aux_var)*sqrt(gamma1_aux_var)*corr_param

#Create a mean vector and a variance-covariance matrix (i.e., sigma_matrix)
mu_vector    =c(alpha1_aux_mu,beta1_aux_mu,lambda_aux_mu,pers_aux_mu,gob_aux_mu,gamma1_aux_mu)
tau          =c(alpha1_aux_var,beta1_aux_var,lambda_aux_var,pers_aux_var,gob_aux_var,gamma1_aux_var) #var vector
...
colnames(true.parms)<-c("alpha1","beta1","lambda","pers","go_bias","gamma1")
...
true.parms[,6]<-inv.logit(true.parms[,6])
...
print(paste('true gamma1 population parm is', gamma1_mu,'true sample mean is',mean(true.parms[,6])))
...
print(paste('true gamma1 population parm is', gamma1_aux_mu,'true sample mean is',mean(auxiliary_parameters[,6])))
...
print(paste('true gamma1 aux var parm is', gamma1_aux_var,'true sample mean is',var(auxiliary_parameters[,6])))
...
df<- lapply(1:Nsubj,function(s)           {
  ...
  df_subj=cbind(subj=rep(s,Ntrl),
                trial=(1:Ntrl),
                sim.model1(Ntrl=Ntrl,x=true.parms[s,],rndwlk=rndwlk))
})
...
resp1=t(sapply(1:Nsubj,function(subj) {df[df$subj==subj,'resp1']}))
resp2=t(sapply(1:Nsubj,function(subj) {df[df$subj==subj,'resp2']}))
...
resp1=arrange_MD_forStan(mydata=resp1,newval=0)  
resp2=arrange_MD_forStan(mydata=resp2,newval=0)
...
#fit Null model 
model_data[['Nparam']]=6
fit_model1 <- stan(file ="Hierarchical_model1.stan" ,
                 data = model_data,
                 warmup = 1000,
                 iter = 2000,#20000
                 chains=4,
...
gamma1_aux_mu_recovered = summary(rl_fit , pars=c("mu[6]"))$summary[,1]
...
gamma1_aux_sigma_recovered = summary(rl_fit , pars=c("tau[6]"))$summary[,1]
...
print(paste('true gamma1 population parm is', gamma1_mu,'true sample mean is',mean(true.parms[,6]),'and recovered is',gamma1_aux_mu_recovered))
...
print(paste('true gamma1 aux population parm is', gamma1_aux_mu,'true sample mean is',mean(auxiliary_parameters[,6]),'and recovered is',gamma1_aux_mu_recovered))
...
print(paste('true gamma1 aux var parm is', gamma1_aux_var,'true sample mean is',var(auxiliary_parameters[,6]),'and recovered is',gamma1_aux_sigma_recovered))
...
gamma1_individual_recovered=summary(rl_fit , pars=c("gamma1"))$summary[,1]
...
plot(true.parms[,6],(gamma1_individual_recovered))

diag(round(cor(cbind(true.parms[,1],true.parms[,2],true.parms[,3],true.parms[,4],true.parms[,5],true.parms[,6]),cbind(alpha1_individual_recovered,beta1_individual_recovered,lambda_individual_recovered,pers_individual_recovered,gob_individual_recovered,gamma1_individual_recovered)),2))
round(cor(cbind(alpha1_individual_recovered,beta1_individual_recovered,lambda_individual_recovered,pers_individual_recovered,gob_individual_recovered,gamma1_individual_recovered),cbind(alpha1_individual_recovered,beta1_individual_recovered,lambda_individual_recovered,pers_individual_recovered,gob_individual_recovered,gamma1_individual_recovered)),2)
round(cor(cbind(true.parms[,1],true.parms[,2],true.parms[,3],true.parms[,4],true.parms[,5],true.parms[,6]),cbind(true.parms[,1],true.parms[,2],true.parms[,3],true.parms[,4],true.parms[,5],true.parms[,6])),2)

#additional
summary(rl_fit, pars=c("L_Omega"))$summary
L_Omega_recovered = matrix(nrow=Nparam,ncol=Nparam)
L_Omega_recovered[1,c(1:Nparam)]<-summary(rl_fit, pars=c("L_Omega"))$summary[c(1:6),1]
L_Omega_recovered[2,c(1:Nparam)]<-summary(rl_fit, pars=c("L_Omega"))$summary[c(7:12),1]
L_Omega_recovered[3,c(1:Nparam)]<-summary(rl_fit, pars=c("L_Omega"))$summary[c(13:18),1]
L_Omega_recovered[4,c(1:Nparam)]<-summary(rl_fit, pars=c("L_Omega"))$summary[c(19:24),1]
L_Omega_recovered[5,c(1:Nparam)]<-summary(rl_fit, pars=c("L_Omega"))$summary[c(25:30),1]
L_Omega_recovered[6,c(1:Nparam)]<-summary(rl_fit, pars=c("L_Omega"))$summary[c(31:36),1]
...
sigma_matrix_recovered = matrix(nrow=Nparam,ncol=Nparam)
sigma_matrix_recovered[1,c(1:Nparam)]<-summary(rl_fit, pars=c("sigma_matrix"))$summary[c(1:6),1]
sigma_matrix_recovered[2,c(1:Nparam)]<-summary(rl_fit, pars=c("sigma_matrix"))$summary[c(7:12),1]
sigma_matrix_recovered[3,c(1:Nparam)]<-summary(rl_fit, pars=c("sigma_matrix"))$summary[c(13:18),1]
sigma_matrix_recovered[4,c(1:Nparam)]<-summary(rl_fit, pars=c("sigma_matrix"))$summary[c(19:24),1]
sigma_matrix_recovered[5,c(1:Nparam)]<-summary(rl_fit, pars=c("sigma_matrix"))$summary[c(25:30),1]
sigma_matrix_recovered[6,c(1:Nparam)]<-summary(rl_fit, pars=c("sigma_matrix"))$summary[c(31:36),1]
round(sigma_matrix_recovered,1)
sigma_matrix


# Visualization -----------------------------------------------------------

traceplot(fit_model1, pars = c("mu"), inc_warmup = FALSE)
plot(fit_model1, plotfun = "hist", pars = c("mu", "tau"))
